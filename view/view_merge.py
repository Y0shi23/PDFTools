from controller.merge3 import MergePDFs
from PyQt6 import QtWidgets as qtw
from threading import Thread
from typing import Final
from typing import overload

class ViewMerge(qtw.QWidget):
    
  def __init__(self):
    super().__init__()
    
    # 説明文
    self.info_label : qtw.QLabel = qtw.QLabel("複数のPDFを1つにまとめます", self)
  ...
  def select_input_directory(self) -> None:
    directory = qtw.QFileDialog.getExistingDirectory(self, caption="Select Input Directory")
  ...
  # execute
  def start_pdf_merging(self) -> None:
    ...
    mergePdfs : MergePDFs = MergePDFs(input_directory, output_directory)
    thread : Thread = Thread(target=mergePdfs.merge_all_pdfs, args=(self.textbox,))
    thread.start()